...
def ruta_con_parada(G, origen, parada, destino):
    """Calcula la ruta óptima origen->parada->destino usando pesos (Dijkstra).
    Devuelve tupla (ruta_completa, peso_total, ruta1, peso1, ruta2, peso2).
    """
    # comprobar conectividad de cada tramo
...
if __name__ == "__main__":
    # Elegimos dos nodos que sabemos están conectados (si quieres otros, cámbialos)
    origen = 12
    destino = 47
    # Parada elegida manualmente
    parada = 22  
    ...
    print("Tramo 1:", ruta1, f"(peso={peso1})")
    print("Tramo 2:", ruta2, f"(peso={peso2})")
    ...
    aristas_tramo1 = list(zip(ruta1[:-1], ruta1[1:]))
    aristas_tramo2 = list(zip(ruta2[:-1], ruta2[1:]))

    # Dibujar nodos de la ruta
    nx.draw_networkx_nodes(G, pos, nodelist=ruta_completa, node_color='red', node_size=400)

    # Dibujar cada tramo con color distinto y ancho mayor
    nx.draw_networkx_edges(G, pos, edgelist=aristas_tramo1, edge_color='blue', width=3, arrows=True, arrowstyle='-|>', arrowsize=20)
    nx.draw_networkx_edges(G, pos, edgelist=aristas_tramo2, edge_color='green', width=3, arrows=True, arrowstyle='-|>', arrowsize=20)

    # resaltar la parada
    nx.draw_networkx_nodes(G, pos, nodelist=[parada], node_color='yellow', node_size=550)

    # Etiquetas de pesos solo si caben
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('ruta_con_parada.png', dpi=150)
    plt.show()

    print("\nImagen guardada como: ruta_con_parada.png")

    # Nota: si deseas cambiar los nodos, modifica las variables 'origen', 'destino' o define 'parada' manualmente.
